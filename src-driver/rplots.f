...
C---- blade solidity
      ...
      CMNR = SEVAL(YMNR,CHR(1,N),W1,YRC,NRC)
...
C---- blade angles and twist
      ...
      BTIP = BETAR(NRC,N)
      B2   = BETAR(2,N)
      BEXT = BHUB - YRC(1,N)*(BHUB-B2)/(YRC(1,N)-YRC(2,N))
      BTWIST = BHUB - BTIP
      CALL SPLINE(BETAR(1,N),W2,YRC,NRC)
      BETAMR = SEVAL(YMNR,BETAR(1,N),W2,YRC,NRC)
C
C---- character size for axis numbers, labels
      CS  = 1.0*CSIZE
      CSL = CS*1.4
...
C
       ...
       CALL PLSUBS(XL2 ,YL,CS,'twist'     ,0.0,5,PLCHAR)
       CALL PLMATH(XL2 ,YL,CS,'b    = '   ,0.0,7)
...
C
       ...
       CALL PLNUMB(999.,YL,CS, ADISK(N)  ,0.0,6)
       CALL PLCHAR(XL2 ,YL,CS,'Atip = '  ,0.0,7)
       CALL PLNUMB(999.,YL,CS, ATIP(N)   ,0.0,6)
       CALL PLSUBS(XL3 ,YL,0.8*CS,'mean' ,0.0,4,PLCHAR)
       CALL PLMATH(XL3 ,YL,CS,'s    = '  ,0.0,7)
...
C------ projection chords in front and behind radial axis
        W1(I) =  XPAXIS     *SINB*CHR(I,N)/RTIP(N)
        W2(I) = (XPAXIS-1.0)*SINB*CHR(I,N)/RTIP(N)
C
        W3(I) =  XPAXIS     *COSB*CHR(I,N)/RTIP(N)
        W4(I) = (XPAXIS-1.0)*COSB*CHR(I,N)/RTIP(N)
C
        W5(I) =  XPAXIS     *CHR(I,N)/RTIP(N)
        W6(I) = (XPAXIS-1.0)*CHR(I,N)/RTIP(N)
C
        CHUMAX = MAX(CHUMAX,     CHR(I,N)/RTIP(N))
        CHLMAX = MAX(CHLMAX,SINB*CHR(I,N)/RTIP(N))
   10 CONTINUE
C
...
C------ plot blade shape
        CALL NEWPEN(4)
        CALL XYLINE(NRC,XI,W5,0.0,1.0,0.0,1.0,1)
        CALL XYLINE(NRC,XI,W6,0.0,1.0,0.0,1.0,1)
C
C------ plot hub line
        CALL NEWPEN(2)
        CALL PLOT(XIHUB,CHR(1,N)* XPAXIS     ,3)
        CALL PLOT(XIHUB,CHR(1,N)*(XPAXIS-1.0),2)
C
...
C------ plot blade shape
        CALL NEWPEN(4)
        CALL XYLINE(NRC,XI,W1,0.0,1.0,0.0,1.0,1)
        CALL XYLINE(NRC,XI,W2,0.0,1.0,0.0,1.0,1)
C
C------ plot hub surface
        CALL NEWPEN(2)
        XLIN(1) = XIHUB
...
C------ plot blade shape
        CALL NEWPEN(4)
        CALL XYLINE(NRC,XI,W3,0.0,1.0,0.0,1.0,1)
        CALL XYLINE(NRC,XI,W4,0.0,1.0,0.0,1.0,1)
C
C---- label for plot view
        XT = XI(1) + 1.0*CS
        YT = -0.35*YSPACE
...
C------ plot blade shape
        CALL NEWPEN(4)
        CALL XYLINE(NRC,W3,W1,0.0,1.0,0.0,1.0,1)
        CALL XYLINE(NRC,W4,W2,0.0,1.0,0.0,1.0,1)
C
        I = 1
...
C------ and tip-angle lines
        DX = DS*COS(BETAR(NRC,N))
        DY = DS*SIN(BETAR(NRC,N))
        CALL PLOT(-DX,-DY,3)
        CALL PLOT( DX, DY,2)
C
C---- label for plot view
...
C
      ...
      CALL PLMATH(XL4 ,YL,CS,'s   = ' ,0.0,6)
      CALL PLNUMB(999.,YL,CS, SIGMR   ,0.0,4)
      CALL PLSUBS(XL5 ,YL,CS,'twist'  ,0.0,5,PLCHAR)
      CALL PLMATH(XL5 ,YL,CS,'b   = ' ,0.0,6)
      CALL PLNUMB(999.,YL,CS, BTWIST/DTR  ,0.0,3)
C
      YL = YL - 2.5*CS
      CALL PLCHAR(XL1 ,YL,CS,'Vm/s= ' ,0.0,6)
      CALL PLNUMB(999.,YL,CS, QINF    ,0.0,3)
      CALL PLCHAR(XL2 ,YL,CS,'RPM = ' ,0.0,6)
      CALL PLNUMB(999.,YL,CS, RPM     ,0.0,1)
      CALL PLCHAR(XL3, YL,CS,'   R  ' ,0.0,6)
      CALL PLMATH(XL3, YL,CS,'  W   ' ,0.0,6)
      CALL PLCHAR(XL3, YL,CS,' /    ' ,0.0,6)
      CALL PLCHAR(XL3, YL,CS,'V   = ' ,0.0,6)
      CALL PLNUMB(999.,YL,CS, ADV     ,0.0,4)
      CALL PLCHAR(XL4 ,YL,CS,'h km= ' ,0.0,6)
...
c      CALL PLNUMB(999.,YL,CS, TDIM/4.45,0.0,1)
      CALL PLCHAR(XL1 ,YL,CS,'Ttot N= ' ,0.0,8)
      CALL PLNUMB(999.,YL,CS, TDIM,0.0,3)
      CALL PLCHAR(XL2 ,YL,CS,'Tduct=  ' ,0.0,8)
      CALL PLNUMB(999.,YL,CS, TDUCT,0.0,3)
...
c      CALL PLNUMB(999.,YL,CS, TDIM/746.,0.0,1)
      ...
      CALL PLNUMB(999.,YL,CS, FOM,0.0,3)
C
      YL = YL - 2.5*CS
      CALL PLSUBS(XL1 ,YL,CS,'C'      ,0.0,1,PLCHAR)
      CALL PLCHAR(XL1 ,YL,CS,'T   = ' ,0.0,6)
      CALL PLNUMB(999.,YL,CS,TC,0.0,3)
      CALL PLSUBS(XL2 ,YL,CS,'C'      ,0.0,1,PLCHAR)
      CALL PLCHAR(XL2 ,YL,CS,'P   = ' ,0.0,6)
      CALL PLNUMB(999.,YL,CS,PC,0.0,3)
...
C
      YL = YL - 2.5*CS
      CALL PLCHAR(XL1 ,YL,CS,'T(N)  = ' ,0.0,8)
      CALL PLNUMB(999.,YL,CS, TTOTR(N),0.0,3)
      CALL PLCHAR(XL2 ,YL,CS,'P(kW) = ' ,0.0,8)
      CALL PLNUMB(999.,YL,CS, PTOTR(N)/1000.,0.0,4)
      CALL PLCHAR(XL3 ,YL,CS,'Q(Nm) = ' ,0.0,8)
      CALL PLNUMB(999.,YL,CS, QTOTR(N),0.0,3)
      CALL PLCHAR(XL4 ,YL,CS,'Vavg= ' ,0.0,6)
...
C---- Blade radial station and circulation for plotting
      ...
      END DO
      CALL MINMAX(NRC,GAMB,GMIN,GMAX)
      CALL MINMAX(NRC,CLR(1,N),CLMIN,CLMAX)
C
...
C
      IF(IRTYPE(N).EQ.2) THEN
       CALL NEWPEN(4)
       CALL XYLINE(NRC,XI,CLR(1,N),0.0,1.0,CLOFF,CLFAC,1)     
       CALL NEWPEN(3)
       IL = 2*(NRC/3)
       XPLT =                 XI(IL)
...
C
      CALL NEWPEN(4)
      CALL XYLINE(NRC,XI,GAMB,0.0,1.0,GOFF,GFAC,2)
      IL = NRC/5
      CALL NEWPEN(2)
...
C
      CALL NEWPEN(4)
      CALL XYLINE(NRC,XI,MACHR(1,N),0.0,1.0,EOFF,EFAC,4)
      IL = 4*NRC/5
      CALL NEWPEN(3)
      XPLT =                 XI(IL)
...
C
      CALL NEWPEN(4)
      CALL XYLINE(NRC,XI,W1,0.0,1.0,0.0,EFAC,3)
      IL = 2*NRC/5
      CALL NEWPEN(3)
      XPLT =                 XI(IL)
...
C---- Vaxial vs r/R
      ...
      XL = XI(IA) + 1.5*CS
      IF(LREL) THEN
       CALL XYLINE(NRC,XI,W1,0.0,1.0,0.0, WFAC,1)
      ...
      IF(LABS) THEN
        ILIN = 1
        IF(LREL) ILIN = 2
        CALL XYLINE(NRC,XI,W4,0.0,1.0,0.0, WFAC,ILIN)
...
C---- Vduct (axial) vs r/R
      ...
      XL = XI(IB) + 1.5*CS
      IF(LREL) THEN
        CALL XYLINE(NRC,XI,W3,0.0,1.0,0.0, WFAC,1)
      ...
      IF(LABS) THEN
        ILIN = 1
        IF(LREL) ILIN = 2
        CALL XYLINE(NRC,XI,W6,0.0,1.0,0.0, WFAC,ILIN)
...
C     
      ...
      XL = XI(IB) + 1.5*CS
      IF(LREL) THEN
        CALL XYLINE(NRC,XI,W2,0.0,1.0,0.0, WFAC,1)
      ...
      IF(LABS) THEN
       ILIN = 1
       IF(LREL) ILIN = 2
       CALL XYLINE(NRC,XI,W5,0.0,1.0,0.0, WFAC,ILIN)
...
C
        ...
        YAXLEN = YSPAN*YSF2D
C
        XLEGND   = 1.0*XAXLEN - FLOAT(NCHLEGEND)*CH3
        YLEGND   = 1.0*YAXLEN
        IF(YVAR.EQ.'D') THEN
          XLEGND = 0.1*XAXLEN
        ENDIF
